...
{
	int i;
	int currentrow = 0;//sound.getRow();
	int currentorder = 0;//sound.getOrder();
	int currenttime = (int)kello.getMilliseconds();

	//nollataan "tällä framella aktiiviseksi"-muuttuja. 
	active = false;

	//käydään läpi mahdolliset osumat
	...
	for (i=0;i<count;i++)
	{
		//vanha/juuri löydetty osuma. Lasketaan interpolaatio
		if (activated[i]==true)
		{
