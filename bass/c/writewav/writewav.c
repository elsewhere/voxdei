...
void main(int argc, char **argv)
{
	...
	if ((chan=BASS_StreamCreateFile(FALSE,argv[1],0,0,BASS_STREAM_DECODE))
		...
		for (p=0;BASS_MusicGetAttribute(chan,BASS_MUSIC_ATTRIB_VOL_CHAN+p)!=-1;p++); // count channels
		printf("MOD music \"%s\" [%u chans, %u orders]",
			BASS_ChannelGetTags(chan,BASS_TAG_MUSIC_NAME),p,BASS_MusicGetOrders(chan));
		pos=BASS_ChannelGetLength(chan);
	}
