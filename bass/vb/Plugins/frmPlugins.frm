...
'///////////////////////////////////////////////////////////////////////////
' frmPlugins.frm - Copyright (c) 2006 (: JOBnik! :) [Arthur Aminov, ISRAEL]
'                                                   [http://www.jobnik.org]
'                                                   [  jobnik@jobnik.org  ]
'
' BASS plugin test
...
Private Sub btnOpen_Click()
    ...
    chan = BASS_StreamCreateFile(BASSFALSE, cmdOpenFile.filename, 0, 0, BASS_SAMPLE_LOOP)
    ...
    Dim bytes As Long
    bytes = BASS_ChannelGetLength(chan)
...
Private Sub sldPosition_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    Call BASS_ChannelSetPosition(chan, BASS_ChannelSeconds2Bytes(chan, sldPosition.value))  ' set the position
    tmrPlugins.Enabled = True
End Sub

Private Sub tmrPlugins_Timer()
    sldPosition.value = BASS_ChannelBytes2Seconds(chan, BASS_ChannelGetPosition(chan)) ' update position
End Sub
