...
'/////////////////////////////////////////////////////////////////////////
' frmSynth.frm - Copyright (c) 2006 (: JOBnik! :) [Arthur Aminov, ISRAEL]
'                                                 [http://www.jobnik.org]
'                                                 [  jobnik@jobnik.org  ]
'
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If (KeyCode = vbKeySubtract Or KeyCode = vbKeyAdd) Then
        ...
        If (KeyCode = vbKeySubtract) Then
            buflen = BASS_SetConfig(BASS_CONFIG_BUFFER, buflen - 1) ' smaller buffer
        Else
            buflen = BASS_SetConfig(BASS_CONFIG_BUFFER, buflen + 1) ' larger buffer
        End If
        lblWinTxt.Caption = "using a " & buflen & "ms buffer"
        str = BASS_StreamCreate(44100, 2, 0, AddressOf WriteStream, 0)
        ' set effects on the new stream
        For r = 0 To 9
            If (fx(r)) Then fx(r) = BASS_ChannelSetFX(str, BASS_FX_CHORUS + r, 0)
        Next r
        Call BASS_ChannelPlay(str, BASSFALSE)
    End If
    ...
    If (KeyCode >= vbKeyF1 And KeyCode <= vbKeyF9) Then
        ...
        Else
            ...
            fx(r) = BASS_ChannelSetFX(str, BASS_FX_CHORUS + r, 0)
