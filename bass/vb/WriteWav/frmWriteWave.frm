...
'//////////////////////////////////////////////////////////////////////////////////
' frmWriteWave.frm - Copyright (c) 2002-2006 (: JOBnik! :) [Arthur Aminov, ISRAEL]
'                                                          [http://www.jobnik.org]
'                                                          [  jobnik@jobnik.org  ]
'
...
Private Sub btnLoadFile_Click()
    ...
    If chan = 0 Then chan = BASS_StreamCreateURL(cmd.filename, 0, BASS_STREAM_DECODE Or BASS_STREAM_BLOCK, 0, 0)
    ...
    If chan = 0 Then
        chan = BASS_MusicLoad(BASSFALSE, cmd.filename, 0, 0, BASS_MUSIC_DECODE Or BASS_MUSIC_RAMP Or BASS_MUSIC_PRESCAN, 0)
        If chan = 0 Then
            ' not a MOD either
        ...
        Else
            p = 0
            While (BASS_MusicGetAttribute(chan, BASS_MUSIC_ATTRIB_VOL_CHAN + p) <> -1)  ' count channels
                p = p + 1
            Wend
            lblStrFile.Caption = "MOD music """ & VBStrFromAnsiPtr(BASS_ChannelGetTags(chan, BASS_TAG_MUSIC_NAME)) & _
                                    """ [" & p & " chans, " & BASS_MusicGetOrders(chan) & " orders]"
            pos = BASS_ChannelGetLength(chan)
        End If
    End If
...
Private Sub btnConvert_Click()
    ...
    pos = 0
    Call BASS_ChannelSetPosition(chan, 0)
End Sub
