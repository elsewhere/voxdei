...
begin
  ...
  chn := BASS_StreamCreateFile(FALSE, PChar(ParamStr(1)), 0, 0, BASS_SAMPLE_LOOP);
  if (chn = 0) then
    chn := BASS_StreamCreateURL(PChar(ParamStr(1)), 0, BASS_SAMPLE_LOOP, nil, 0);
  if (chn <> 0) then
  begin
    pos := BASS_ChannelGetLength(chn);
  ...
  begin
    ...
    chn := BASS_MusicLoad(FALSE, PChar(ParamStr(1)), 0, 0, BASS_MUSIC_LOOP or BASS_MUSIC_RAMPS or BASS_MUSIC_PRESCAN, 0);
    if (chn = 0) then
      // not a MOD either
      Error('Can''t play the file');
    // count channels
    a := 0;
    while (BASS_MusicGetAttribute(chn, BASS_MUSIC_ATTRIB_VOL_CHAN + a) <> DWORD(-1)) do
      a := a + 1;
    Write('playing MOD music "' + BASS_ChannelGetTags(chn, BASS_TAG_MUSIC_NAME) + '" [' + IntToStr(a) + ' chans, ' + IntToStr(BASS_MusicGetOrders(chn)) + ' orders]');
    pos := BASS_ChannelGetLength(chn);
    ismod := True;
  end;
  ...
  begin
    // display some stuff and wait a bit
		...
		if (ismod) then
    begin
			pos := BASS_MusicGetOrderPosition(chn);
  ...
  BASS_ChannelSlideAttributes(chn,-1,-2,-101,200);
  while (BASS_ChannelIsSliding(chn) <> 0) do
    Sleep(1);
